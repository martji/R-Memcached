...
public class TPCW_Database {
	...
	static String jdbcPath = "jdbc:mysql://localhost:3306/tpcw2?user="
			+ userName + "&password=" + userPasswd + "&autoReconnect=true";
		...
		try {
			...
			synchronized (Cart.class) {
				...
				PreparedStatement insert_cart = con
						.prepareStatement("INSERT into shopping_cart (sc_id, sc_time) "
								+ "VALUES ((SELECT COUNT(*) FROM (SELECT b.* from shopping_cart as b) as a),"
								+ "NOW())");
	...
	public static void refreshSession(int C_ID) {
		try {
			...
			PreparedStatement updateLogin = con
					.prepareStatement("UPDATE customer SET c_login = CURRENT_TIMESTAMP, c_expiration = DATE_ADD(CURRENT_TIMESTAMP , INTERVAL 2 HOUR) WHERE c_id = ?");
	...
	public static void enterCCXact(Connection con,
		...
		try {
			...
			// System.out.println(Thread.currentThread().getName()+" Enter CCXact: Failed to write with o_id= "+o_id);
			ex.printStackTrace();
		}
	}
	...
	public static int getNumberofOrders() {
		Connection con = getConnection();
		PreparedStatement get_max_id2;
		int noid = 0;
		try {
			get_max_id2 = con.prepareStatement("SELECT COUNT(*) FROM orders");

			ResultSet rs2 = get_max_id2.executeQuery();
			rs2.next();
			noid = rs2.getInt(1);
			get_max_id2.close();
			con.commit();
	...
	public static int enterOrder(Connection con, int customer_id, Cart cart,
		...
		try {
			PreparedStatement insert_row = con
					.prepareStatement("INSERT into orders (o_id, o_c_id, o_date, o_sub_total, "
			...
			synchronized (Order.class) {
				...
				// rs.next();
				// o_id = rs.getInt(1) + 1;
				o_id = IDGenerator.getOID();
				// System.out.println(Thread.currentThread().getName()+" Enter Oder: I have just read number of orders is "+(o_id-1));
				// rs.close();
				// System.out.println(Thread.currentThread().getName()+" Enter Oder: I will write o_id="+o_id);
				insert_row.setInt(1, o_id);
				insert_row.executeUpdate();
				con.commit();
				// PreparedStatement get_max_id2 = con.prepareStatement
				// ("SELECT COUNT(*) FROM orders");
				// ResultSet rs2 = get_max_id2.executeQuery();
				// rs2.next();
				// int noid = rs2.getInt(1);
				// System.out.println(Thread.currentThread().getName()+" Enter Oder: After commit a new record, number of orders is "+noid);
				// get_max_id.close();
			}

			insert_row.close();
		} catch (java.lang.Exception ex) {
			// System.out.println(Thread.currentThread().getName()+" Enter Oder: failed to write o_id="+o_id);
			ex.printStackTrace();
	...
	public static void addOrderLine(Connection con, int ol_id, int ol_o_id,
		...
		try {
			...
			// System.out.println(Thread.currentThread().getName()+" Add Oder Line: I will write o_id= "+ol_o_id);
			insert_row.executeUpdate();
			insert_row.close();
		} catch (java.lang.Exception ex) {
			// System.out.println(Thread.currentThread().getName()+" Add Oder Line: Failed to  write o_id= "+ol_o_id);
			ex.printStackTrace();
		}
	}
